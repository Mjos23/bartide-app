...
September 21 update: the actual DigitalOcean proposal validated the two-service configuration at $10/month, and app `bb80369f-0289-44b6-9f25-d84386e9babc` was created in the verified default project. Both Linux images built and started against Supabase. HTTP readiness probes failed; switching to TCP readiness made deployment `9b20c973-da4c-4924-bc26-177eb2ec07f5` active. Public `/health` returns 200. Diagnostic deployment observed transport peers in `100.127.0.0/16`, valid client-IP headers and HTTPS. The provider-specific ingress check now includes RFC6598 shared address space `100.64.0.0/10`; boundary regression tests cover both edges and IPv4-mapped addresses. Hosted re-verification remains required.

Optional `ReverseProxy__Diagnostics=true` logs rejection reasons and transport peers plus a short client-address fingerprint on accepted requests, without credentials, cookies, bodies or client-header values. It defaults off and must be disabled after hosted header-overwrite/internal-forwarding verification. TCP readiness is not proof of public HTTP correctness.
